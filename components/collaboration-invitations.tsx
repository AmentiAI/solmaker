...
export function CollaborationInvitations() {
  ...
  if (loading) {
    return (
      <div className="cosmic-card border-2 border-[#ff6b35]/30 rounded-xl p-6 shadow-lg">
  ...
  return (
    <div className="cosmic-card border-2 border-[#ff6b35]/30 rounded-xl p-6 shadow-lg">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="px-3 py-1 bg-[#ff6b35]/20 text-[#ff6b35] text-sm font-semibold rounded-full border border-[#ff6b35]/30">
      ...
      <div className="space-y-4">
        {invitations.map((invitation) => (
          <div
            key={invitation.id}
            className="cosmic-card border-2 border-[#ff6b35]/30 rounded-xl p-4"
          >
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-white/60">
                  <span>
                    Invited by: <strong className="text-white/80">{invitation.inviter_username || invitation.invited_by.substring(0, 10) + '...'}</strong>
                  </span>
                  <span>
                    Role: <strong className="capitalize text-white/80">{invitation.role}</strong>
                  </span>
                  <span>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="px-4 py-2 bg-[#00d4ff] hover:bg-[#00b8e6] text-white rounded-lg font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={processing === invitation.id}
                className="px-4 py-2 cosmic-card border border-[#00d4ff]/30 hover:border-[#00d4ff]/50 text-white/70 hover:text-white rounded-lg font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {processing === invitation.id ? 'Processing...' : 'Decline'}
              </button>
              <Link
                href={`/collections/${invitation.collection_id}`}
                className="px-4 py-2 bg-[#ff6b35] hover:bg-[#ff5722] text-white rounded-lg font-semibold transition-colors"
              >
                View Collection
              </Link>
            </div>
