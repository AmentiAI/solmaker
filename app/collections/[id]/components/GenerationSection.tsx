  ...
  return (
    <div>
      {(queuedJobs > 0 || processingJobs > 0) && (
        <div className="mb-6 cosmic-card border-2 border-[#00d4ff]/50 rounded-lg p-4 sm:p-6 shadow-lg">
      ...
      {hasActiveFilters && (
        <div className="mb-6 cosmic-card border-2 border-[#ff6b35]/50 rounded-lg p-4">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[#ff6b35] text-lg">⚠️</span>
                <h3 className="text-base sm:text-lg font-bold text-[#ff6b35]">Trait Filters Active</h3>
              </div>
            ...
            <button onClick={onClearFilters} className="bg-[#ff6b35] hover:bg-[#ff5722] text-white px-4 py-2 rounded font-semibold text-sm transition-colors whitespace-nowrap shadow-lg shadow-[#ff6b35]/20">
              Clear
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center rounded-lg border border-[#00d4ff]/30 cosmic-card overflow-hidden">
            <button
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs sm:text-sm font-semibold transition-all duration-200 ${
                useClassicMode
                  ? 'bg-gradient-to-r from-[#ff6b35] to-[#ff5722] text-white shadow-lg shadow-[#ff6b35]/20'
                  : 'text-white/60 hover:text-white/80 bg-transparent'
              } disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              Model 1.0
        ...
        {collection?.collection_status === 'launchpad' && !isUserAdmin ? (
          <div className="cosmic-card border border-[#ff6b35]/50 rounded-lg p-3 w-full sm:w-auto">
            <p className="text-sm text-[#ff6b35]">⚠️ Generation is disabled for collections on launchpad.</p>
          </div>
        ) : (
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
            <button onClick={onGenerate} disabled={generating || layers.length === 0} className={`${hasActiveFilters ? 'bg-[#ff6b35] hover:bg-[#ff5722] border-2 border-[#00d4ff]/50 shadow-lg shadow-[#ff6b35]/20' : 'bg-[#00d4ff] hover:bg-[#00b8e6] shadow-lg shadow-[#00d4ff]/20'} text-white px-4 sm:px-6 py-2 sm:py-3 rounded disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base font-bold transition-all duration-200 w-full sm:w-auto drop-shadow-lg`}>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <input 
                ...
                className="flex-1 sm:w-20 border border-[#00d4ff]/30 rounded px-2 py-1 cosmic-card text-white text-center focus:border-[#00d4ff] focus:outline-none focus:ring-2 focus:ring-[#00d4ff]/20" 
                disabled={generating || layers.length === 0} 
              />
            </div>
          </div>
      ...
      {layers.length > 0 && (
        <div className="cosmic-card border border-[#00d4ff]/30 rounded-lg p-4 mb-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4">
            {layers.map((layer) => (
              <div key={layer.id}>
                ...
                <select value={traitFilters[layer.name] || ''} onChange={(e) => onFilterChange(layer.name, e.target.value)} className="w-full border border-[#00d4ff]/30 rounded px-2 py-1 text-sm cosmic-card text-white focus:border-[#00d4ff] focus:outline-none focus:ring-2 focus:ring-[#00d4ff]/20">
          ...
          <button onClick={onClearFilters} className="text-sm text-[#00d4ff] hover:text-[#00b8e6] transition-colors">Clear all filters</button>
        </div>
      )}
